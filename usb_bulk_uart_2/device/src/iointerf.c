...
void handle_usb_packet_rx_cb(usbd_device *usbd_dev, uint8_t ep __attribute__((unused)))
{
    ...
    if (rx_len > 0) 
    {
        //if data has been received, copy it into data buffer
        ...
        //Then update data_size
        data_buffer.data_size += rx_len;

        //If data size exceeds the maximum allowable value set is_full to true and
        //the endpoint to NAK. The "buffer full" event will be handled by 
        //handle_main_tasks()
...
void handle_main_tasks(void)
{
    ...
    {
        /*
         * This section should try to send continuously
         * data over UART. However, this is not an easy
         * task because a packet may arrive while the
         * buffer is being manipulated and the program
         * would loose track of how much data was stored
         * inside of it.
        */
    }
}
